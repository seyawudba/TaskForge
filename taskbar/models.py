...
class Project(Timer):
    ...
    status = models.CharField(
        max_length=50, db_index=True, default=StatusChoices.TASK_TO_DO, choices=StatusChoices.choices
    )
...
class Membership(Timer):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, db_index=True, on_delete=models.CASCADE, related_name="memberships"
    )
    role = models.CharField(
        max_length=50, db_index=True, default=WorkspaceMembershipRole.MEMBER, choices=WorkspaceMembershipRole.choices
    )
...
class Task(Timer):
    ...
    severity = models.CharField(
        max_length=50, db_index=True, choices=SeverityChoices.choices, default=SeverityChoices.MEDIUM
    )
